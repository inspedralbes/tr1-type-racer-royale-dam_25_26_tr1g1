<template>
  <aside class="panel">
    ...
    <ul>
      ...
      <li
        v-for="name in Array.from(seen.angles).sort()"
        :key="'ang-' + name"
      ...
      <li
        v-for="name in Array.from(seen.inclinations).sort()"
        :key="'inc-' + name"
        style="margin-left: 12px"
      >
      ...
      <li
        v-for="name in Array.from(seen.velocities).sort()"
        :key="'vel-' + name"
        style="margin-left: 12px"
      >
      ...
      <li
        v-for="name in Array.from(seen.normalized).sort()"
        :key="'norm-' + name"
        style="margin-left: 12px"
      >
        <template v-if="typeof features?.normalized?.[name] === 'object'">
          {{ name }}:
          <span
            v-for="(v, k2) in features?.normalized?.[name]"
            :key="name + '-' + k2"
          >
            {{ k2 }}={{ fmt(v, 3)
            }}<span
              v-if="
                k2 !== Object.keys(features?.normalized?.[name]).slice(-1)[0]
              "
              >,
            </span>
          </span>
